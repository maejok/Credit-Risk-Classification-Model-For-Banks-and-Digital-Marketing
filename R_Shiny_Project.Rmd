...
We built a browser-based user interface using Shiny for banks and digital marketers to easily visualize predicted outcomes. In its current implementation, the application enables clients to make numerical inputs and obtain instant outputs and to visualize the predictors.
...
#Dropt `Months since last delinquent` 
```{r loan2}
...
```
```{r}
plot(loan2$`Current Loan Amount`[loan2$`Current Loan Amount`<999999]~loan2$`Monthly Debt`[loan2$`Current Loan Amount`<999999])
```
...
where X.beta is the linear combination of the predictors X and beta are the coeficients.
...
preds_binomial = ifelse(preds > .4, 1 , 0)
